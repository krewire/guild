...
1. **Load vision first** — read `internal/docs/project-vision.md` (5-line matrix) and `internal/docs/specs/index.md` (planned vs shipped) to ground the plan in the unified workload matrix. Load the specific `KWF-*` spec for the slice you touch.
...
5. **For complex designs**, write a spec in `internal/docs/specs/<project>/` (centralized in `krewire/internal`) following the spec template; never `docs/specs/` in the old per-repo location.
